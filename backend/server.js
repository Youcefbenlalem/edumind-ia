...
Regles absolues:
- Reponds TOUJOURS selon la methodologie algerienne officielle
- Si la matiere est Arabe ou Education Islamique, reponds en arabe
- Sinon reponds en francais
- Structure ta reponse: definition, explication claire, exemple algerien concret, regle a retenir
- Utilise les memes methodes, plans et vocabulaire vus dans les manuels algeriens
- Sois pedagogique, encourageant et precis`;
...
app.post('/api/ia/quiz', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `Tu es un professeur expert du programme scolaire algerien officiel (ONEC).
Tu generes des questions de quiz STRICTEMENT conformes au programme algerien.
Tu reponds UNIQUEMENT en JSON valide, sans markdown, sans texte avant ou apres le JSON.`;

    let userPrompt;
    if (isMathPhys) {
      userPrompt = `Cree 4 exercices de ${matiere} pour: ${ctx}
Ecris les expressions mathematiques en texte simple: utilise / pour fractions, ^ pour puissances, racine() pour racines.
...
${arabe ? 'Les questions et reponses doivent etre en arabe.' : 'Les questions et reponses doivent etre en francais.'}
Retourne ce JSON exact:
{
  "questions": [
...
app.get('/api/health', (_, res) => res.json({ status: 'ok', message: 'EDUMIND IA Backend Running (Groq) 🧠' }));

app.listen(process.env.PORT || 5000, () => {
  console.log(`🚀 EDUMIND Backend (Groq) running on port ${process.env.PORT || 5000}`);
});
