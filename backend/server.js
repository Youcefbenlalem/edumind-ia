...
const callClaude = async (system, userMsg, maxTokens = 2000) => {
  ...
  if (!apiKey) throw new Error('ANTHROPIC_API_KEY manquante sur le serveur');
...
app.post('/api/ia/chat', auth, async (req, res) => {
  try {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) return res.status(500).json({ message: 'Clé API Claude manquante sur le serveur. Ajoute ANTHROPIC_API_KEY dans Render.' });
...
3. Structure tes réponses: définition → explication → exemple algérien → règle à retenir
4. Utilise des exemples du contexte algérien (prénoms algériens, villes algériennes, situations locales)
5. Pour math/physique: résous étape par étape selon la méthode des manuels algériens
6. Sois encourageant et bienveillant comme un bon professeur algérien`;
...
app.post('/api/ia/quiz', auth, async (req, res) => {
  try {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) return res.status(500).json({ message: 'Clé API Claude manquante' });
...
Types selon le programme algérien: calcul, démonstration, problème appliqué.
Format JSON exact:
...
app.get('/api/users/search', auth, async (req, res) => {
  try {
    ...
    const users = await User.find({
      _id: { $ne: req.user._id },
      $or: [{ firstName: { $regex: q.trim(), $options: 'i' } }, { lastName: { $regex: q.trim(), $options: 'i' } }]
...
app.post('/api/friends/request', auth, async (req, res) => {
  try {
    ...
    if (req.user.friends.map(f => f.toString()).includes(toUserId)) return res.status(400).json({ message: 'Vous êtes déjà amis' });
